...
class ResNetModel(object):
    ...
    def inference(self, x):
        ...
        # post-net
        avg_pool = tf.reduce_mean(s5, reduction_indices=[1, 2], name='avg_pool1')
...
def visit_network(visit):
    ...
    return s5
def get_net_output(fc_image, classNum, KEEP_PROB):
    with tf.variable_scope("fc"):
        image_drop = dropout(fc_image, KEEP_PROB)
        net_output = fc(image_drop, num_units_out=classNum)
    return net_output
